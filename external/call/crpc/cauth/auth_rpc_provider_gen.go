...
func (p *authRpcProvider) Auth(req rpccall.RpcReqWrapper[AuthReq]) (rpccall.RpcRespWrapper[AuthResp], error) {
	...
	if err := cp.Call("/auth", reqForCall, &resp, 1 * time.Second); err != nil {
		return resp, err
	}
	return resp, nil
}
...
func (p *authRpcProvider) Clear(req rpccall.RpcReqWrapper[ClearReq]) (rpccall.RpcRespSimple, error) {
	...
	if err := cp.Call("/clear", reqForCall, &resp, 2 * time.Second); err != nil {
		return resp, err
	}
	return resp, nil
}
...
func (p *authRpcProvider) UpdateToken(req rpccall.RpcReqWrapper[UpdateReq]) (rpccall.RpcRespSimple, error) {
	...
	if err := cp.Call("/update_token", reqForCall, &resp, 1 * time.Second); err != nil {
		return resp, err
	}
	return resp, nil
}

func (p *authRpcProvider) Ping(req PingReq) (PingResp, error) {
	cp := p.acquireClientProxy()
	var resp PingResp
	var reqForCall interface{}
	...
	if err := cp.Call("/ping", reqForCall, &resp, 1 * time.Second); err != nil {
		return resp, err
	}
	return resp, nil
}
